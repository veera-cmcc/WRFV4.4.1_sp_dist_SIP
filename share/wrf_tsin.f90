    ...
    CALL model_to_grid_config_rec ( grid%id , model_config_rec , config_flags ) 
       ...
       IF ( wrf_dm_on_monitor() ) THEN
          ...
          IF (exists) THEN
             ...
             IF ( iunit <= 0 ) THEN
                CALL wrf_error_fatal3("wrf_tsin.b",44,&
'Error in wrf_tsin: could not find a free Fortran unit.')
             ...
             IF (istatus == 0) THEN
                ...
                elseif(index(IorLat,'I').ne.0 .and. index(JorLon,'J').ne.0) then
                    ...
                    CALL wrf_error_fatal3("wrf_tsin.b",68,&
'Error in wrf_tsin: Header line requires either LAT LON or I J')
                endif
                ...
                DO WHILE (istatus == 0)
                   ...
                   IF (istatus > 0) THEN
                      WRITE(errmess, FMT='(I4)') grid%ntsloc + 4   
                                                                   
                      CALL wrf_message('Error in tslist, line '//trim(errmess))
                      CALL wrf_error_fatal3("wrf_tsin.b",92,&

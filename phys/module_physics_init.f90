...
CONTAINS
   ...
   SUBROUTINE phy_init ( id, config_flags, DT, restart, zfull, zhalf,     &
                         ...
                         z_at_q, inv_dens, qnwfa2d, qnbca2d,     & 
                         scalar, num_sc,                         & 
                         re_cloud, re_ice, re_snow,              & 
                         has_reqc, has_reqi, has_reqs,           & 
                         PHB,PH,P,PB,QV,CCNTY,QDCN,QTCN,QCCN,    & 
                         QRCN,QNIN,                              & 
                         ...
                         iopt_crop,                                       & 
                         iopt_irr, iopt_irrm,                             &   
                         IRNUMSI, IRNUMMI, IRNUMFI, IRWATSI, IRWATMI,     &   
                         IRWATFI, IRELOSS, IRSIVOL, IRMIVOL, IRFIVOL,     &   
                         IRRSPLH, qtdrain,                                &
                         gecros_state,                                    & 
                         XSAIXY, LAI,                                     & 
   ...
   REAL,     DIMENSION(ims:ime,jms:jme), INTENT(INOUT) :: qnwfa2d, qnbca2d       
  ...
  REAL,     DIMENSION( ims:ime , kms:kme , jms:jme ) , INTENT(OUT), OPTIONAL :: &
   ...
   REAL, OPTIONAL, DIMENSION( ims:ime, 1:urban_map_zdf, jms:jme ),INTENT(INOUT) :: DG_URB3D 
   ...
   INTEGER, OPTIONAL, INTENT(IN) :: CCNTY                                               
   REAL, OPTIONAL, INTENT(IN), DIMENSION(ims:ime,kms:kme,jms:jme) :: PHB,PH,P,PB,QV     
   REAL, INTENT(INOUT), DIMENSION(ims:ime,kms:kme,jms:jme) :: QDCN,QTCN,QCCN,QRCN,QNIN  
   ...
   if (( (config_flags%ra_lw_physics .eq. RRTMG_LWSCHEME ) .or. &
       ...
       ( (config_flags%ra_sw_physics .eq. RRTMG_SWSCHEME ) .or. &
        ...
        (config_flags%mp_physics  .eq. THOMPSON .or.        &
         ...
         config_flags%mp_physics  .eq. P3_1CAT_3MOM   ) ) then    
      has_reqc = 1
      has_reqi = 1
      has_reqs = 1
   endif
 ...
 ENDIF
  ...
  ENDIF
   ...
   CALL mp_init(RAINNC,SNOWNC,GRAUPELNC,config_flags,restart,warm_rain,          &
                ...
                F_ICE_PHY,F_RAIN_PHY,F_RIMEF_PHY,               &
                PHB,PH,P,PB,QV,XLAND,CCNTY,QDCN,QTCN,QCCN,QRCN, & 
                QNIN,                                           & 
                mp_restart_state,tbpvs_state,tbpvs0_state,      &
                allowed_to_read, start_of_simulation,           &
        ...
        DO ls=1,luseas
          ...
          DO LC=1,LUCATS
            IF(found_lu)THEN
              ...
              IF(LC.NE.LI)CALL wrf_error_fatal3("<stdin>",1812,&
          ...
          IF ((SF_URBAN_PHYSICS.eq.1).OR.(SF_URBAN_PHYSICS.EQ.2).OR.(SF_URBAN_PHYSICS.EQ.3)) THEN
             ...
             IF ( PRESENT( FRC_URB2D ) .AND. PRESENT( UTYPE_URB2D )) THEN
                ...
                CALL urban_var_init(ISURBAN,TSK,TSLB,TMN,IVGTYP,               & 
                              ...
                              B_T_BEP,B_Q_BEP,B_E_BEP,DLG_BEP,                 & 
                              DL_U_BEP,SF_BEP,VL_BEP,                          & 
                              FRC_URB2D, UTYPE_URB2D,config_flags%use_wudapt_lcz)  
          

               max_utype_urb2d = maxval(UTYPE_URB2D)*1.0
               max_utype_urb2d = wrf_dm_max_real(max_utype_urb2d)
               IF (config_flags%use_wudapt_lcz.eq.0 .and. max_utype_urb2d.gt.3.0) THEN  
                 CALL wrf_error_fatal &
                 ('USING 10 WUDAPT LCZ WITHOUT URBPARM_LCZ.TBL. SET USE_WUDAPT_LCZ=1')
               ENDIF
               IF (config_flags%use_wudapt_lcz.eq.1 .and. max_utype_urb2d.le.3.0) THEN  
                 CALL wrf_error_fatal &
...
'NoahMP does not work with NLCD data. Stop.' )
          ...
          IF ((SF_URBAN_PHYSICS.eq.1).OR.(SF_URBAN_PHYSICS.EQ.2).OR.(SF_URBAN_PHYSICS.EQ.3)) THEN
             IF ( PRESENT( FRC_URB2D ) .AND. PRESENT( UTYPE_URB2D )) THEN
               ...
               IF (config_flags%use_wudapt_lcz.eq.0 .and. max_utype_urb2d.gt.3.) THEN  
                 CALL wrf_error_fatal &
                 ('USING 10 WUDAPT LCZ WITHOUT URBPARM_LCZ.TBL. SET USE_WUDAPT_LCZ=1')
               ENDIF
               IF (config_flags%use_wudapt_lcz.eq.1 .and. max_utype_urb2d.le.3.) THEN  
                 CALL wrf_error_fatal &
                 ('USING URBPARM_LCZ.TBL WITH OLD 3 URBAN CLASSES. SET USE_WUDAPT_LCZ=0')
               ENDIF
...
'arguments not present for calling camzmscheme' )
  ...
  INTEGER :: i,j,itf,jtf
   ...
   SUBROUTINE mp_init(RAINNC,SNOWNC,GRAUPELNC,config_flags,restart,warm_rain,      &
                      ...
                      MPDT, DT, DX, DY, LOWLYR,                   & 
                      F_ICE_PHY,F_RAIN_PHY,F_RIMEF_PHY,           & 
                      PHB,PH,P,PB,QV,XLAND,CCNTY,QDCN,QTCN,QCCN,  & 
                      QRCN,QNIN,                                  & 
                      mp_restart_state,tbpvs_state,tbpvs0_state,   & 
                      allowed_to_read, start_of_simulation,       &
   ...
   REAL,     DIMENSION( ims:ime , kms:kme, jms:jme ) , INTENT(INOUT) :: &
                                  F_ICE_PHY,F_RAIN_PHY,F_RIMEF_PHY
   INTEGER, OPTIONAL, INTENT(IN) :: CCNTY                                              
   REAL, OPTIONAL, DIMENSION(ims:ime,jms:jme), INTENT(IN) :: XLAND                     
   REAL, OPTIONAL, INTENT(IN), DIMENSION(ims:ime,kms:kme,jms:jme) :: PHB,PH,P,PB,QV    
   REAL, INTENT(INOUT), DIMENSION(ims:ime,kms:kme,jms:jme) :: QDCN,QTCN,QCCN,QRCN,QNIN 
   REAL , DIMENSION(:) ,INTENT(INOUT)  :: mp_restart_state,tbpvs_state,tbpvs0_state
   LOGICAL , INTENT(IN)  :: allowed_to_read

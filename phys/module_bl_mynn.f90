    ...
    SELECT CASE(bl_mynn_cloudpdf)
      ...
      CASE (2, -2)
        ...
        DO k = kts,kte-1
           ...
           q1(k) = qmq(k) / sgm(k)  
           cldfra_bl1D(K) = MAX(0., MIN(1., 0.5+0.36*ATAN(1.55*q1(k)))) 
        ...
        DO k = kts,kte-1
           ...
           IF (q1k < 0.) THEN        
              ...
              ql_ice   = low_weight * sgm(k)*EXP(1.1*q1k-1.6) &  
                  + (1.-low_weight) * sgm(k)*EXP(1.1*q1k-2.8)
           ELSE IF (q1k > 2.) THEN   
              ql_water = sgm(k)*q1k
           ...
           IF ( qc(k) + qi(k) > 0.0 ) THEN 
              IF ( qi(k) == 0.0 ) THEN       
                liq_frac = 1.0  
              ELSE IF ( qc(k) == 0.0 ) THEN  
                liq_frac = 0.0
              ELSE IF ( (qc(k) >= 1.E-10) .AND. (qi(k) >= 1.E-10) ) THEN  

...
CONTAINS
   ...
   SUBROUTINE surface_driver(                                         &
     ...
     &          ,smois,smstav,smstot,snoalb                           &
     
     
     &          ,REFKDT_2D_LSM, SLOPE_2D_LSM                                  &
     
     &          ,snow,snowc,snowh,stepbl   &
     &          ,smcrel                                               &
   ...
   REAL, DIMENSION( ims:ime , jms:jme ), INTENT(INOUT):: qtiledrain,ZWATBLE2D  
   ...
   REAL,    OPTIONAL, INTENT(IN) :: soiltstep  
  ...
  real,  intent(inout),optional :: t_veg24(ims:ime,1:maxpatch,jms:jme)      
  ...
  REAL, DIMENSION( ims:ime, jms:jme ) ::  REFKDT_2D_LSM  
  REAL, DIMENSION( ims:ime, jms:jme ) ::  SLOPE_2D_LSM   
...
'Lacking arguments for P-X LSM in surface driver')
     ...
     IF (MYJ) call wrf_error_fatal3("<stdin>",3571,&
...
'CLM land surface model need num_soil_layers=10')
       endif
...
'Lacking arguments for SSIB in surface driver')
 ...
 subroutine qnsesfc_seaice_wrapper(ITIMESTEP,HT,DZ,      &
   ...
   SUBROUTINE sfclayrev_seaice_wrapper(U3D,V3D,T3D,QV3D,P3D,dz8w,     &
     ...
     U10_HOLD(its:ite,jts:jte)    = U10(its:ite,jts:jte) 
     V10_HOLD(its:ite,jts:jte)    = V10(its:ite,jts:jte) 

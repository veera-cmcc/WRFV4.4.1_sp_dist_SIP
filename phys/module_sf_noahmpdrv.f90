...
CONTAINS
 ...
 INTEGER,    DIMENSION( ims:ime,          jms:jme ), INTENT(INOUT) ::  PGSXY
    ...
    REAL,    DIMENSION( ims:ime,          jms:jme ), INTENT(INOUT) ::  sfcheadrt,INFXSRT,soldrain,qtiledrain,ZWATBLE2D   
    REAL,    DIMENSION( ims:ime, jms:jme ), INTENT(IN)          ::  REFKDT_2D_LSM 
    REAL,    DIMENSION( ims:ime, jms:jme ), INTENT(IN)          ::  SLOPE_2D_LSM  
    ...
    REAL,    DIMENSION( ims:ime,          jms:jme ), INTENT(INOUT) ::  QRAINXY   
    ...
    REAL,    DIMENSION( ims:ime,          jms:jme ), INTENT(OUT  ) ::  PAHXY     
    REAL,    DIMENSION( ims:ime,          jms:jme ), INTENT(OUT  ) ::  PAHGXY    
    REAL,    DIMENSION( ims:ime,          jms:jme ), INTENT(OUT  ) ::  PAHBXY    
    REAL,    DIMENSION( ims:ime,          jms:jme ), INTENT(OUT  ) ::  PAHVXY    
    ...
    REAL,    DIMENSION( ims:ime,          jms:jme ), INTENT(OUT  ) ::  QMELTXY   
    REAL,    DIMENSION( ims:ime,          jms:jme ), INTENT(OUT  ) ::  PONDINGXY
    REAL,    DIMENSION( ims:ime,          jms:jme ), INTENT(OUT  ) ::  FPICEXY    
    REAL,    DIMENSION( ims:ime,          jms:jme ), INTENT(OUT  ) ::  RAINLSM     
    REAL,    DIMENSION( ims:ime,          jms:jme ), INTENT(OUT  ) ::  SNOWLSM     
    ...
    REAL,    DIMENSION( ims:ime,          jms:jme ),  INTENT(IN)    :: IRFRACT    
    REAL,    DIMENSION( ims:ime,          jms:jme ),  INTENT(IN)    :: SIFRACT    
    REAL,    DIMENSION( ims:ime,          jms:jme ),  INTENT(IN)    :: MIFRACT    
    REAL,    DIMENSION( ims:ime,          jms:jme ),  INTENT(IN)    :: FIFRACT    
    ...
    REAL                                :: ACC_QINSUR
    REAL                                :: ACC_QSEVA
    REAL, DIMENSION( 1:NSOIL)           :: ACC_ETRANI       
    REAL                                :: EFLXB
    REAL                                :: XMF
    ...
    soil_update_steps = nint(soiltstep/DT)  
   ...
   ILOOP : DO I = its, ite
    ...
    ELSE
	 ...
	 PRCPNONC  = PRCPNONC + PRCPOTHR
       ...
       QTLDRN                = 0.                           
       TDFRACMP              = TD_FRACTION(I,J)             
       WATBLED               = ZWATBLE2D (I,J)              
       ...
       parameters%refkdt = REFKDT_2D_LSM (I,J)         
       parameters%slope  = SLOPE_2D_LSM (I,J)         
    ...
    ELSE
	    ...
	    PRCPCONV, PRCPNONC, PRCPSHCV, PRCPSNOW, PRCPGRPL, PRCPHAIL, & 
            ...
            IRRFRA  , SIFAC   , MIFAC   , FIFAC   , LLANDUSE,           & 
            ALBOLD  , SNEQVO  ,                                         & 
	    ...
	    CHLEAF  , CHUC    , CHV2    , CHB2    , FPICE   , PAHV    , & 
            ...
            QEVAC   , QDEWC   , QMELT   ,                               & 
            RAININ  , SNOWIN  , ACC_SSOIL, ACC_QINSUR, ACC_QSEVA      , & 
   ...
   ENDIF 
            ...
            soldrain(i,j) = max(RUNSB, 0.0)        
            INFXSRT(i,j) = RUNSF   
            qtiledrain (i,j) = QTLDRN   
	     ...
	     SMSTAV   (I,J)                = 0.0  
             SMSTOT   (I,J)                = 0.0  
             SFCRUNOFF(I,J)                = SFCRUNOFF(I,J) + RUNSF  
             UDRUNOFF (I,J)                = UDRUNOFF(I,J)  + RUNSB  
             QTDRAIN  (I,J)                = QTDRAIN (I,J)  + QTLDRN 
             IF ( SALB > -999 ) THEN
                ALBEDO(I,J)                = SALB
             ENDIF
             SNOWC    (I,J)                = FSNO
	     ...
	     PGSXY    (I,J) = PGS
             ...
             IRELOSS(I,J)                  = IRELOSS(I,J)+(EIRR*DT) 
             IRRSPLH(I,J)                  = IRRSPLH(I,J)+(FIRR*DT) 
...
SUBROUTINE TRANSFER_MP_PARAMETERS(VEGTYPE,SOILTYPE,SLOPETYPE,SOILCOLOR,CROPTYPE,parameters)
  ...
  parameters%SCFFAC = SCFFAC_TABLE(VEGTYPE)       
  ...
  END IF
   ...
   parameters%RSURF_EXP  =   RSURF_EXP_TABLE
    ...
    parameters%IRR_FRAC   = IRR_FRAC_TABLE      
    parameters%IRR_HAR    = IRR_HAR_TABLE       
    parameters%IRR_LAI    = IRR_LAI_TABLE       
    parameters%IRR_MAD    = IRR_MAD_TABLE       
    parameters%FILOSS     = FILOSS_TABLE        
    parameters%SPRIR_RATE = SPRIR_RATE_TABLE    
    parameters%MICIR_RATE = MICIR_RATE_TABLE    
    parameters%FIRTFAC    = FIRTFAC_TABLE       
    parameters%IR_RAIN    = IR_RAIN_TABLE       
       ...
       DO J = jts,jtf
          DO I = its,itf
             ...
             IF ( SNOW( i,j ) > 2000.0 ) THEN
               SNOWH(I,J) = SNOWH(I,J) * 2000.0 / SNOW(I,J)      
               SNOW (I,J) = 2000.0                               
             ENDIF
          ENDDO
       ENDDO
	     ...
	     if(iopt_crop == 2) then
	       cropcat    (i,j) = 0
             ...
             if(iopt_irr >= 1 .and. iopt_irr <= 3) then
                if(iopt_irrm == 0 .or. iopt_irrm ==1) then       
                   irnumsi(i,j) = 0
                   irwatsi(i,j) = 0.
                   ireloss(i,j) = 0.
                   irrsplh(i,j) = 0.     
                else if (iopt_irrm == 0 .or. iopt_irrm ==2) then 
                   irnummi(i,j) = 0
                   irwatmi(i,j) = 0.
                   irmivol(i,j) = 0.
                else if (iopt_irrm == 0 .or. iopt_irrm ==3) then 
                   irnumfi(i,j) = 0
                   irwatfi(i,j) = 0.
                   irfivol(i,j) = 0.
                end if

...
"Error reading qr_acr_qgV3.dat. Aborting because force_read_thompson is .true.")
              ENDIF
              CLOSE(63)
            ELSE
              IF( force_read_thompson ) THEN
                CALL wrf_error_fatal3("<stdin>",3710,&
"Error opening qr_acr_qgV3.dat. Aborting because force_read_thompson is .true.")
              ENDIF
            ENDIF
          ELSE
            INQUIRE(63,OPENED=lopen)
...
"Non-existent qr_acr_qgV3.dat. Aborting because force_read_thompson is .true.")
          ENDIF
        ENDIF
      ENDIF

...
MODULE module_mp_nssl_2mom
  ...
  integer, private :: interval_sedi_vt = 1 
  ...
  real,private,parameter :: meltfac     = 1.0 
  ...
  real    :: charging_border = 4000. 
      ...
      double precision :: gamxinflu(0:nqiacrratio,ialpstart:nqiacralpha,12,2) 
    ...
    DATA mmgraupvt(:,3) / 0.67819, 0.63789, 0.62197, 0.61240, 0.60572, 0.60066, 0.59663, 0.59330, 0.59048 /
      ...
      real, parameter :: cwradn = 2.0e-6, xcradmn = cwradn    
...
SUBROUTINE nssl_2mom_driver(qv, qc, qr, qi, qs, qh, qhl, ccw, crw, cci, csw, chw, chl,  &
      ...
      real, dimension(ims:ime, kms:kme, jms:jme),  optional, intent(inout)::                   &
                            induc,noninduc,noninducp,noninducn  
  ...
  real :: cinchange, t7max,testmax,wmax
   ...
   LOGICAL, optional, intent(in) :: wetscav_on
     ...
     IF ( itimestep == 1 .and. .not. invertccn .and.  present( cn ) ) THEN
      ...
      ELSEIF ( cn((ite+its)/2,(kte+kts)/2,(jte+jts)/2) < 10.0 ) THEN 
        DO jy = jts,jte
         DO kz = kts,kte
          DO ix = its,ite
            cn(ix,kz,jy) = qccn
...
20         CONTINUE
     ...
     &   timesed1,timesed2,timesed3,zmaxsed,timesetvt) 
      ...
      DO ix = ixb,ixe
       ...
       rhovtzx(kz,ix) = Sqrt(rho00*Min(1.0/0.05, db1inv(ix,kz))) 
      ENDDO
      ENDDO
     ...
     &          .and. t0(ix,jy,kz) .gt. z0(ix,kz,l) ) THEN
      ...
      real, parameter :: xgms=xdnh*0.523599*(300.e-6)**3    
      real, parameter :: cwmas09 = 1000.*0.523599*(2.*9.e-6)**3 
     ...
     &  ((1 + rnux)**3.3333333333333335*(2 + rnux)*Gamma_sp(1 + rnux))
      ...
      if ( qx(mgs,ls) .gt. qxmin(ls) ) then
        IF ( ipconc .ge. 4 ) THEN
         ...
         IF ( sssflg < 0 .and. temcg(mgs) > Abs(sssflg) ) THEN 
            vtxbar(mgs,ls,2) = vtxbar(mgs,ls,1)
 ...
 1200 continue
     ...
     &                  .and. an(ix,jy,kz,ls) > an(ix,jy,kz,lr) .and. an(ix,jy,kz,lr) > qsmin) THEN
             ...
             IF ( an(ix,jy,kz,lns) .gt. 1.e-7 ) THEN
               ...
               IF ( qxw > qsmin .or. iusewetsnow >= 2 ) THEN 
   ...
   implicit none
      ...
      real, parameter :: cwmas30 = 1000.*0.523599*(2.*30.e-6)**3 
      real, parameter :: cwmas20 = 1000.*0.523599*(2.*20.e-6)**3 
      integer nxmpb,nzmpb,nxz
      integer mgs,ngs,numgs,inumgs
      parameter (ngs=500)
      integer ngscnt,igs(ngs),kgs(ngs)
 ...
 609       dqv  = -(ss1 - 1.)*d1*dtcon
     ...
     &             ((temg(mgs) - cbw)**2))
         ...
         ssmx = ssmxinit
          ...
          IF ( ssf(mgs) > ssmx  .and. ssf(mgs) < 20.0 .and. ccnc(mgs) > 0.05*cwnccn(mgs)) THEN 
  ...
  616 IF ( ssf(mgs) .LE. SUPCB .AND. wvel(mgs) .GT. 0. ) GO TO 631 
     ...
     &         ( wvel(mgs)*DSSDZ) )  )
      ...
      ELSEIF ( irenuc == 5 ) THEN 
       ...
       CN(mgs) =  Min( cnuc(mgs),  CCNE0*cnuc(mgs)**(2./(2.+cck))*Max(0.0,wvel(mgs))**cnexp )
        ...
        temp1 = (theta0(mgs)+thetap(mgs))*pk(mgs) 
         ...
         ltemq = Min( nqsat, Max(1,ltemq) )
          ...
          IF ( c1 > 0. ) THEN
            ssf(mgs) = Max(0.0, 100.*((qv0(mgs) + qwvp(mgs))/c1 - 1.0) )  
          ELSE
            ssf(mgs) = 0.0
          ENDIF
          

       CN(mgs) =   Max( cn(mgs), cnuc(mgs)*Min(ssf2kmax, ssf(mgs)**cck) ) 
       ...
       ELSE
         CN(mgs) =  Min( cn(mgs), cnuc(mgs) - ccna(mgs) ) 
       ENDIF
       ...
       dcloud = 1000.*dcrit**3*Pi/6.
         ...
         tmp = Max(0.0,  rho0(mgs)*qx(mgs,lc)/dcloud - cx(mgs,lc)) 
         ...
         ltemq = Min( nqsat, Max(1,ltemq) )
          ...
          ENDIF
           ...
           IF ( ccncuf(mgs) > 0.0 .and. ssf(mgs) > ssmxuf .and. ssmax(mgs) > ssmxuf ) THEN
            CNuf(mgs) =  Min( ccncuf(mgs), CCNE0*ccncuf(mgs)**(2./(2.+cck))*Max(0.0,wvel(mgs))**cnexp )
...
29998 continue
 ...
 2200 continue
   ...
   END SUBROUTINE NUCOND
     ...
     & )
      ...
      real, parameter :: rvt = 0.104  
      real, parameter :: kfrag = 1.0e-6 
      real, parameter :: mfrag = 1.0e-10 
      double precision cautn(ngs), rh(ngs), nh(ngs)
      real ex1, ft, rhoinv(ngs)
      double precision ec0(ngs)
      
      real ac1,bc, taus, c1,d1,e1,f1,p380,tmp,tmp1,tmp2,tmp3,tmp4,tmp5,temp3 
      real dw,dwr
      ...
      real qrcev(ngs)
      real qrshr(ngs)
      real fsw(ngs),fhw(ngs),fhlw(ngs),ffw(ngs) 
      real fswmax(ngs),fhwmax(ngs),fhlwmax(ngs) 
      real qhcnf(ngs) 
      real :: qhlcnh(ngs)
     ...
     &             ((3.+alpharaut)*(2.+alpharaut)*(1.+alpharaut))
      ...
      ELSEIF ( ibinnum == 2 ) THEN
        numdiam = 2 
        mltdiam(1) = mltdiam1/6. 
        mltdiam(2) = mltdiam1/2. 
      ELSEIF ( ibinnum > 2 ) THEN
      ...
      IF ( numshedregimes == 2 ) THEN
        ...
        mltdiam(ndiam+2) = mltdiam3 
        mltdiam(ndiam+3) = mltdiam4 
 ...
 1100 continue
      ...
      rho0(mgs) = dn(igs(mgs),jy,kgs(mgs))
      rhoinv(mgs) = 1.0/rho0(mgs)
      rhovt(mgs) = Sqrt(rho00/Max(0.05,rho0(mgs))) 
      pi0(mgs) = p2(igs(mgs),jy,kgs(mgs)) + pinit(kgs(mgs))
      temg(mgs) = t0(igs(mgs),jy,kgs(mgs))
      ...
      do mgs = 1,ngscnt
        ...
        DO ic = lc,lhab
        dab0lh(mgs,il,ic) =  dab0(il,ic) 
        dab1lh(mgs,il,ic) =  dab1(il,ic) 
        ENDDO
      ENDDO
      end do
     ...
     &                 cwmasn,cwmasx,cwradn,cnina,cimn,cimx,   &
     &                 itype1,itype2,temcg,infdo,alpha,0,axx,bxx) 
     ...
     &        ((alpha(mgs,lc) + 2.)*xv(mgs,lc)/(alpha(mgs,lc) + 1.) + xv(mgs,lr))/rho0(mgs) 
         ELSE
     ...
     &  , qrmxd(mgs))
      ...
      do mgs = 1,ngscnt
      csacs(mgs) = 0.0
      IF ( qx(mgs,ls) > qxmin(ls) .and. ess(mgs) .gt. 0.0 ) THEN 

        IF ( iessec0flag == 0 ) THEN
          ec0(mgs) = 1.0
        ELSE
          tmp = xv(mgs,ls)/(xvmx(ls)*Max(1.,100./Min(100.,xdn(mgs,ls)))) 
      ...
      csacs(mgs) = ec0(mgs)*rvt*aa2*ess(mgs)*cx(mgs,ls)**2*Min( xv(mgs,ls), 4.*pii/3.*essrmax**3 ) 
     ...
     &  , qrmxd(mgs))
       ...
       ELSEIF ( ipconc .ge. 3 ) THEN
         ...
         IF ( ibiggopt == 2 .and. imurain == 1 ) THEN 
           ...
           ELSEIF ( dbigg < Max( biggsnowdiam, Max(dfrz,dhmn)) .and. ( ibiggsnow == 1 .or. ibiggsnow == 3 ) ) THEN 
         ...
         qwfrz(mgs) = cwfrz(mgs)*xdn0(lc)*rhoinv(mgs)*(volt + xv(mgs,lc))
          ELSE
            ...
            IF ( .false. .and. usegamxinfcnu ) THEN
              ...
              cwfrz(mgs) = cx(mgs,lc)*Gamxinf(1.+alpha(mgs,lc), ratio)/(dtp*gcnup1) 

              qwfrz(mgs) = cx(mgs,lc)*xdn0(lc)*xv(mgs,lc)*rhoinv(mgs)*Gamxinf(2.+alpha(mgs,lc), ratio)/(dtp*gcnup2) 
            ...
            ELSE
              ...
              cwfrz(mgs) = cx(mgs,lc)*tmp*dtpinv 

              tmp = gaminterp(ratio,alpha(mgs,lc),12,1)

              qwfrz(mgs) = cx(mgs,lc)*xdn0(lc)*xv(mgs,lc)*rhoinv(mgs)*dtpinv*tmp 
     ...
     &        ltest .and. qx(mgs,lh) .gt. hlcnhqmin ) .or. wtest ) THEN 
        ...
        IF ( qhacw(mgs) .gt. 0.0 .and. qhacw(mgs) .gt. qhaci(mgs) .and. temg(mgs) .le. tfr-2.0 ) THEN 
         ...
         IF ( xdia(mgs,lh,3)/dh0 .gt. 0.1 ) THEN 
           ...
           IF ( .not. wtest ) dh0 = Min( dh0, 10.e-3 ) 
           IF ( qx(mgs,lhl) > 0.1e-3 ) dh0 = Max( dh0, xdia(mgs,lhl,3) ) 

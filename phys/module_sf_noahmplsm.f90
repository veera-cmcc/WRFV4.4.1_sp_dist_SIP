...
MODULE MODULE_SF_NOAHMPLSM
  ...
  logical :: calculate_soil
  integer :: soil_update_steps
...
contains
  ...
  REAL                           , INTENT(INOUT) :: ACC_SSOIL   
  REAL                           , INTENT(INOUT) :: ACC_QINSUR  
  REAL                           , INTENT(INOUT) :: ACC_QSEVA   
  REAL, DIMENSION(       1:NSOIL), INTENT(INOUT) :: ACC_ETRANI  
  REAL, DIMENSION(-NSNOW+1:NSOIL), INTENT(  OUT) :: HCPCT       
  REAL                           , INTENT(  OUT) :: EFLXB       
  REAL                           , INTENT(INOUT) :: ACC_DWATER  
  REAL                           , INTENT(INOUT) :: ACC_PRCP    
  REAL                           , INTENT(INOUT) :: ACC_ECAN    
  REAL                           , INTENT(INOUT) :: ACC_ETRAN   
  REAL                           , INTENT(INOUT) :: ACC_EDIR    
  ...
  REAL, INTENT(OUT)                                           :: PAH     
  REAL, INTENT(OUT)   :: QSNSUB
  REAL, INTENT(OUT)   :: QSNFRO
  REAL, INTENT(OUT)   :: QSUBC
  REAL, INTENT(OUT)   :: QFROC
  REAL, INTENT(OUT)   :: QFRZC
  REAL, INTENT(OUT)   :: QMELTC
  REAL, INTENT(OUT)   :: QEVAC
  REAL, INTENT(OUT)   :: QDEWC
  ...
  REAL                        , INTENT(IN)       :: TDFRACMP 


  REAL                        , INTENT(IN)       :: IRRFRA         
  REAL                        , INTENT(IN)       :: SIFRA          
  REAL                        , INTENT(IN)       :: MIFRA          
  REAL                        , INTENT(IN)       :: FIFRA          
  INTEGER                     , INTENT(INOUT)    :: IRCNTSI        
  INTEGER                     , INTENT(INOUT)    :: IRCNTMI        
  INTEGER                     , INTENT(INOUT)    :: IRCNTFI        
  REAL                        , INTENT(INOUT)    :: IRAMTSI        
  REAL                        , INTENT(INOUT)    :: IRAMTMI        
  REAL                        , INTENT(INOUT)    :: IRAMTFI        
  REAL                        , INTENT(INOUT)    :: IRSIRATE       
  REAL                        , INTENT(INOUT)    :: IRMIRATE       
  REAL                        , INTENT(INOUT)    :: IRFIRATE       
  REAL                        , INTENT(INOUT)    :: FIRR           
  REAL                        , INTENT(INOUT)    :: EIRR           
...
"Namelist parameter DVEG unknown") 
    ...
    if (calculate_soil) then
       ...
       IF((CROPLU .EQV. .TRUE.) .AND. (IRRFRA .GE. parameters%IRR_FRAC) .AND. &
          ...
          CALL TRIGGER_IRRIGATION(parameters,NSOIL,ZSOIL,SH2O,FVEG,JULIAN,IRRFRA,LAI, & 
                                  SIFAC,MIFAC,FIFAC,                                & 
                                  IRCNTSI,IRCNTMI,IRCNTFI,                          & 
    ...
    IF((CROPLU .EQV. .TRUE.) .AND. (IRAMTSI .GT. 0.0)) THEN
        CALL SPRINKLER_IRRIGATION(parameters,NSOIL,DT,SH2O,SMC,     & 
                                  SFCTMP,UU,VV,EAIR,SIFAC,          & 
                                  IRAMTSI,IREVPLOS,IRSIRATE)          
        RAIN = RAIN + (IRSIRATE*1000.0/DT) 
        
        FIRR = IREVPLOS*1000.0*HVAP/DT                              
        EIRR = IREVPLOS*1000.0/DT                                   
    END IF
    ...
    QVAP = MAX( FGEV/LATHEAG, 0.0)       
    QDEW = ABS( MIN(FGEV/LATHEAG, 0.0))  
    EDIR = QVAP - QDEW
   ...
   END IF
    ...
    IF(SNOWH <= 1.E-6 .OR. SNEQV <= 1.E-6) THEN
     SNOWH = 0.0
     SNEQV = 0.0
    END IF

    IF(SWDOWN.NE.0.0) THEN
      ALBEDO = FSR / SWDOWN
    ELSE
      ALBEDO = -999.9
    END IF
  ...
  INTEGER, PARAMETER :: NITER = 10 
	...
	ELSE
     ...
     IF(OPT_SNF == 5) THEN
       TDC   = MIN( 50.0, MAX(-50.0,(SFCTMP-TFRZ)) ) 
       IF (SFCTMP > TFRZ) THEN
          LATHEA = HVAP
       ELSE
          LATHEA = HSUB
       ...
       DO ITER = 1, NITER
          ...
          TWET    = TWET - (ESATAIR-EAIR)/ GAMMA_b  
       END DO
       FPICE = 1.0/(1.0+6.99E-5*exp(2.0*(TWET+3.97))) 
     ENDIF
...
ENDIF   
     ...
     IF(parameters%HVT> 0.0 .AND. parameters%HVT <= 1.0) THEN          
  ...
  REAL                :: ICEDRIP 
	 ...
	 ICEDRIP = MAX(0.0,CANICE) * (FV+FT)    
         ICEDRIP = MIN(CANICE/DT + QINTS, ICEDRIP)      
  ...
  REAL, INTENT(IN)   :: PAHB    
  REAL                           , INTENT(IN) :: FIRR   
  REAL                           , INTENT(IN) :: CANHS  
  REAL                           , INTENT(IN) :: IRMIRATE  
  REAL                           , INTENT(IN) :: IRFIRATE  
  REAL                           , INTENT(INOUT) :: ACC_DWATER 
  REAL                           , INTENT(INOUT) :: ACC_PRCP   
  REAL                           , INTENT(INOUT) :: ACC_ECAN   
  REAL                           , INTENT(INOUT) :: ACC_ETRAN  
...
"Energy budget problem in NOAHMP LSM")
   ...
   IF (IST == 1) THEN       
      ...
      ACC_DWATER = ACC_DWATER + (END_WB - BEG_WB)  
      ACC_PRCP   = ACC_PRCP   + PRCP * DT        
      ACC_ECAN   = ACC_ECAN   + ECAN * DT        
      ACC_ETRAN  = ACC_ETRAN  + ETRAN * DT       
      ACC_EDIR   = ACC_EDIR   + EDIR * DT        
 ...
 END SUBROUTINE ERROR
  ...
  CALL  RADIATION (parameters,VEGTYP  ,IST     ,ICE     ,NSOIL   , & 
     ...
     IF (ICE == 1) THEN
       EMG = 0.98*(1.0-FSNO) + parameters%SNOW_EMIS*FSNO  
     ELSE
     ...
     ELSE
       ...
       ELSEIF(OPT_RSF == 2) THEN
         RSURF = FSNO * 1.0 + (1.-FSNO)* EXP(8.25-4.225*BEVAP) 
       ELSEIF(OPT_RSF == 3) THEN
         RSURF = FSNO * 1.0 + (1.-FSNO)* EXP(8.25-6.0  *BEVAP) 
       ENDIF
       ...
       IF(SH2O(1) < 0.01 .and. SNOWH == 0.0) RSURF = 1.E6
...
"STOP in Noah-MP")
    ...
    if(calculate_soil) then
      ...
      CALL TSNOSOI (parameters,ICE     ,NSOIL     ,NSNOW   ,ISNOW   ,IST     , & 
                    TBOT      ,ZSNSO   ,SSOIL_avg ,DF      ,HCPCT   , & 
                    SAG       ,DT_soil ,SNOWH     ,DZSNSO  , & 
                    TG        ,ILOC    ,JLOC      ,                   & 
                    STC       ,EFLXB   )                             
...
100 CONTINUE
  ...
  REAL :: SL                   
        ...
        ALBSND(1)=ALBSNI(1)+parameters%BATS_VIS_DIR*FZEN*(1.0-ALBSNI(1))    
        ALBSND(2)=ALBSNI(2)+parameters%BATS_NIR_DIR*FZEN*(1.0-ALBSNI(2))    
...
"CRITICAL PROBLEM IN MODULE_SF_NOAHMPLSM:VEGEFLUX" )
     ...
     IF(ITER == 1) THEN
       ...
       IF (opt_crop == 2) then
         IF ((GECROS1D(41) .GT. 0).and.(GECROS1D(42) .LT. 0.0)) then   
           Thickness = 0.0
           NROOT = 0
           ROOTD = GECROS1D(33)
           WUL = 0.0
           ...
           DO J = 1,NSOIL
            ...
            if (Thickness .lt. ROOTD/100.0) then
              NROOT = NROOT + 1
            endif
           ENDDO
           ...
           DO J = 1,NROOT
             Thickness = Thickness + DZSNSO (J) 
             if (Thickness .gt. ROOTD/100.0) then
                WUL = WUL + ((ROOTD/100.0-Thickness+DZSNSO(J))*1000.0*(SH2O(J)-parameters%SMCWLT(J)))
             else
           ...
           CALL gecros (JULIAN, DT, 1, RB, RAHC, RAHG+RSURF, FB, SNOWH        , & 
               ...
               TLAIE    = MIN(6.0,TLAI    / FVEG)
               GLAIE    = MIN(6.0,GLAI    / FVEG) 
         ENDIF     
       ENDIF     
     ...
     END IF
        ...
        ELSE
          ...
          CTW  = (1.0-FWET)*(1.0/(RB/(FRSU*GLAIE)+RSSUN) + 1.0/(RB/((1.0-FRSU)*GLAIE)+RSSHA)) 
        ENDIF
        CGW  = 1.0/(RAWG+RSURF)
	...
	END IF
        ...
        HCV = 0.02*VAIE*CWAT + CANLIQ*CWAT/DENH2O + CANICE*CICE/DENICE    
     ...
     IF (SNOWH > 0.05 .AND. TG > TFRZ) THEN
        ...
        IF(OPT_STC == 3) TG  = (1.0-FSNO)*TG + FSNO*TFRZ   
  ...
  TDC(T)   = MIN( 50.0, MAX(-50.0,(T-TFRZ)) )
     ...
     IF (SNOWH > 0.05 .AND. TGB > TFRZ) THEN
          ...
          IF(OPT_STC == 3) TGB  = (1.0-FSNO)*TGB + FSNO*TFRZ  
          IRB = CIR * TGB**4 - EMG*LWDN
          SHB = CSH * (TGB        - SFCTMP)
          EVB = CEV * (ESTG*RHSUR - EAIR )          
...
"STOP in Noah-MP")
  ...
  type (noahmp_parameters), intent(in) :: parameters
    ...
    REAL,    PARAMETER :: EPSU2  = 1.0E-4
    REAL,    PARAMETER :: EPSUST = 0.07
    REAL,    PARAMETER :: EPSIT  = 1.0E-4
    REAL,    PARAMETER :: EPSA   = 1.0E-8
    ...
    ELSE
       ...
       RLOGT = log (ZSLT / ZT)
       USTARK = USTAR * VKRM
       IF(SIMM < 1.0e-6) SIMM = 1.0e-6        
       AKMS = MAX (USTARK / SIMM,CXCH)



       IF(SIMH < 1.0e-6) SIMH = 1.0e-6        
  ...
  type (noahmp_parameters), intent(in) :: parameters
      ...
      REAL :: T
         ...
         CEA = MAX(0.25*EI*parameters%C3PSN+0.40*EI*(1.0-parameters%C3PSN), MIN(EA,EI) )
...
1002      continue
  ...
  REAL,                              INTENT(IN)   :: IRRFRA   
  REAL,                              INTENT(IN)   :: MIFAC    
  REAL,                              INTENT(IN)   :: FIFAC    
  REAL,                              INTENT(INOUT):: IRAMTFI  
  REAL,                              INTENT(INOUT):: IRAMTMI  
  REAL,                              INTENT(INOUT):: IRFIRATE 
  REAL,                              INTENT(INOUT):: IRMIRATE 
  LOGICAL,                           INTENT(IN)   :: CROPLU   
  ...
  real                                           :: qinsur_avg
  real                                           :: qseva_avg
  real, dimension(1:nsoil)                       :: etrani_avg

  REAL, PARAMETER ::  WSLMAX = 5000.0      
  ...
  REAL                           , INTENT(INOUT)    :: sfcheadrt, WATBLED
   ...
   CALL CANWATER (parameters,VEGTYP ,DT     , & 
                  ...
                  CANLIQ ,CANICE ,TV     ,                 & 
                  CMC    ,ECAN   ,ETRAN  , & 
                  FWET , QSUBC, QFROC, QFRZC, QMELTC, QEVAC, QDEWC ) 
   ...
   END IF
    ...
    ACC_QSEVA  = ACC_QSEVA  + QSEVA
    ACC_ETRANI = ACC_ETRANI + ETRANI
    ...
    if(calculate_soil) then
       ...
       IF((CROPLU .EQV. .TRUE.) .AND. (IRAMTFI .GT. 0.0))THEN
          ...
          CALL FLOOD_IRRIGATION(parameters,NSOIL,DT_soil,SH2O,SMC,SICE,FIFAC,& 
                                IRAMTFI,IRFIRATE)                         
          ACC_QINSUR = ACC_QINSUR + IRFIRATE/Dt_soil * soil_update_steps
       END IF
       ...
       IF((CROPLU .EQV. .TRUE.) .AND. (IRAMTMI .GT. 0.0))THEN
          ...
          CALL MICRO_IRRIGATION(parameters,NSOIL,DT_soil,SH2O,SMC,SICE,MIFAC, & 
                                IRAMTMI,IRMIRATE)                          
          SH2O(1) = SH2O(1) + (IRMIRATE/(-1*ZSOIL(1)))
       END IF

       QSEVA_avg  = ACC_QSEVA  / soil_update_steps
       QINSUR_avg = ACC_QINSUR / soil_update_steps 
       ETRANI_avg = ACC_ETRANI / soil_update_steps

       
       IF (IST == 2) THEN                                          
          RUNSRF = 0.0
          IF(WSLAKE >= WSLMAX) RUNSRF = QINSUR_avg*1000.0*Dt_soil  
           WSLAKE = WSLAKE + (QINSUR_avg-QSEVA_avg)*1000.0*Dt_soil - RUNSRF   
       ELSE                                                      
           CALL  SOILWATER (parameters,NSOIL  ,NSNOW  ,DT_soil ,ZSOIL  ,DZSNSO , & 
                            QINSUR_avg,QSEVA_avg,ETRANI_avg,SICE   ,ILOC   , JLOC ,     & 
                            TDFRACMP,DX    ,                                    & 
                            SH2O   ,SMC    ,ZWT    ,VEGTYP ,                    & 
                            SMCWTD, DEEPRECH,                                   & 
          ...
          IF(OPT_RUN == 1) THEN 
             CALL GROUNDWATER (parameters,NSNOW  ,NSOIL  ,DT_soil  ,SICE   ,ZSOIL  , & 
          ...
          IF(OPT_RUN == 5) THEN
             CALL SHALLOWWATERTABLE (parameters,NSNOW  ,NSOIL, ZSOIL, DT_soil   , & 
          ...
          RUNSRF = RUNSRF * dt_soil
          RUNSUB = RUNSUB * dt_soil
          QTLDRN = QTLDRN * dt_soil
       ENDIF

    endif 

    RUNSUB       = RUNSUB + SNOFLOW*DT         
  ...
  REAL                :: CANMAS  
      ...
      IF(CANICE .LE. 1.0E-6) CANICE = 0.0
     


      IF( (CANICE .GT. 0.0) .AND. (CANICE .GE. CANLIQ) ) THEN
  ...
  INTEGER :: NEWNODE            
    ...
    IF(ISNOW == 0 .AND. SNOWH >= 0.025) THEN 
      ISNOW    = -1
      NEWNODE  =  1
      DZSNSO(0)= SNOWH
      SNOWH    = 0.0
  ...
  type (noahmp_parameters), intent(in) :: parameters
   ...
   IF(SNEQV == 0.0) THEN
      SICE(1) =  SICE(1) + (QSNFRO-QSNSUB)*DT/(DZSNSO(1)*1000.0)  
      IF(SICE(1) < 0.0) THEN
         SH2O(1) = SH2O(1) + SICE(1)
         SICE(1) = 0.0
      END IF
  ...
  REAL, PARAMETER :: A = 4.0
    ...
    DO K = 1,NSOIL
       EPORE   = MAX ( 1.0E-4 , ( parameters%SMCMAX(K) - SICE(K) ) )
    ...
    DO ITER = 1, NITER
       ...
       IF(QINSUR > 0.0 .and. OPT_RUN == 6) THEN
          CALL COMPUTE_VIC_SURFRUNOFF(parameters,DTFINE,NSOIL,SMC,ZSOIL,QINSUR,& 
                                      FSAT,RUNSRF,PDDUM)                         
       END IF

       IF (QINSUR > 0.0 .AND. OPT_RUN == 7) THEN
          CALL COMPUTE_XAJ_SURFRUNOFF(parameters,DTFINE,FCR,NSOIL,SMC,ZSOIL,QINSUR,& 
                                      RUNSRF,PDDUM)                                  
       END IF
   ...
   IF(OPT_RUN /= 1) THEN
      ...
      END DO
        ...
        MLIQ(IZ) = MLIQ(IZ) + XS
        RUNSUB   = RUNSUB - XS/DT
        IF(OPT_RUN == 5)DEEPRECH = DEEPRECH - XS*1.0E-3
  ...
  IF(OPT_RUN == 5) THEN
     ...
     ELSE
        ...
        SMCWTD = MAX( MIN(SMCWTD,parameters%SMCMAX(NSOIL)) , 1.0E-4)
  ...
  ENDIF
    ...
    EPORE        = MAX ( 1.0E-4 , ( parameters%SMCMAX(1) - SICE(1) ) )
   ...
   IF(WPLUS > 0.0) THEN
    ...
    EPORE        = MAX ( 1.0E-4 , ( parameters%SMCMAX(NSOIL) - SICE(NSOIL) ) )
  ...
  DO IZ=1,NSOIL-2
    TOP_MOIST     = TOP_MOIST + (SMC(IZ)*(-1.0)*ZSOIL(IZ)) 
    TOP_MAX_MOIST = TOP_MAX_MOIST + (parameters%SMCMAX(IZ)*(-1.0)*ZSOIL(IZ)) 
  END DO
  ...
  I_MAX = (1.0 + parameters%BVIC)*TOP_MAX_MOIST 
  I_0   = I_MAX*(1.0 - (1.0 - ASAT)**(1.0/parameters%BVIC)) 
 ...
 END SUBROUTINE COMPUTE_VIC_SURFRUNOFF
  ...
  SUBROUTINE COMPUTE_XAJ_SURFRUNOFF(parameters,DT,FCR,NSOIL,SMC,ZSOIL,QINSUR,RUNSRF,PDDUM)
    ...
    DO IZ=1,NSOIL-2
       IF ((SMC(IZ)-parameters%SMCREF(IZ)) .GT. 0.0) THEN 
          SM     = SM + (SMC(IZ) - parameters%SMCREF(IZ) )*(-1.0)*ZSOIL(IZ) 
          WM     = WM + (parameters%SMCREF(IZ)*(-1.0)*ZSOIL(IZ))            
       ELSE
  ...
  DP     = QINSUR * DT                                                                   
  I_MAX  = TOP_MAX_MOIST * (parameters%BDVIC+1.0)                                         
...
END SUBROUTINE RR2
  ...
  SUBROUTINE TILE_DRAIN (parameters,NSOIL,SH2O,SMC,SICE,ZSOIL,QTLDRN,DT)
    ...
    DO K = 1, NSOIL
      ...
      END IF
    END DO
      IF (parameters%DRAIN_LAYER_OPT == 0) THEN 
         
         K = parameters%TD_DEPTH
         AVFC(K)  = parameters%SMCREF(K) - SICE (K)
         OVRFC(K) = (SH2O(K) - (parameters%TDSMC_FAC*AVFC(K))) * ZLAYER(K) * 1000.0 
         IF (OVRFC(K) > 0.0) THEN
      ...
      ELSE IF (parameters%DRAIN_LAYER_OPT == 5) THEN 
 ...
 SUBROUTINE TILE_HOOGHOUDT (parameters,WCND,NSOIL,NSNOW,SH2O,SMC,SICE, &
  ...
  IMPLICIT NONE
    ...
    INTEGER                                     :: K
    REAL, DIMENSION(1:NSOIL)                    :: TD_SATZ 
    REAL, DIMENSION(1:NSOIL)                    :: KLATK   
    REAL                                        :: TD_TTSZ 
    REAL                                        :: TD_LQ   
    REAL                                        :: DTOPL   
    REAL                                        :: XX
    ...
    TD_LQ   = 0.0
    TD_TTSZ = 0.0
    TDDC    = parameters%TD_DCOEF* 1000.0 * DT/(24.0 * 3600.0) 
    ...
    OVRFCS = 0.0
    DO K=1, NSOIL
      OVRFC(K) = (SH2O(K) - (parameters%SMCREF(K)-SICE(K))) * ZLAYER(K) * 1000.0 
    ...
    DO K=1, NSOIL
       KLATK(K)= WCND(K)*parameters%KLAT_FAC * DT 
       TD_LQ   = TD_LQ + (TD_SATZ(K) * KLATK(K))
       TD_TTSZ = TD_TTSZ + TD_SATZ(K)
    END DO

    IF (TD_TTSZ .LT. 0.001) TD_TTSZ = 0.001 
    IF (TD_LQ   .LT. 0.001) TD_LQ   = 0.0    
    ...
    IF (TD_HEMD .LE. 0.0) THEN
       QTLDRN = 0.0
    ELSE
       QTLDRN = ((8.0*KLAT*TD_HAIL*TD_HEMD) + (4.0*KLAT*TD_HEMD*TD_HEMD))& 
                 /(parameters%TD_SPAC*parameters%TD_SPAC)
    END IF

    QTLDRN = QTLDRN * 1000.0 
 ...
 IF (TD_X .GT. 0.5) THEN
    DO I=1,45,2
       ...
       TD_FX = TD_FX + TERM
       IF(TERM .LT. 1.0E-6) THEN
         TD_DE = ((PII*TD_L)/8.0)/(LOG(TD_L/(PII*TD_RD))+TD_FX)
         EXIT
       END IF
    END DO
 ELSE IF (TD_X .LT. 1.0E-8) THEN
    TD_DE  = TD_D
 ELSE
 ...
 IF (TD_DE .LT. 0.0 .AND. I .LE. 2) TD_DE = TD_D
 ...
 END SUBROUTINE TD_FINDZWAT 
  ...
  SUBROUTINE TRIGGER_IRRIGATION(parameters,NSOIL,ZSOIL,SH2O,FVEG,                   & 
                                JULIAN,IRRFRA,LAI,                                  & 
                                SIFAC,MIFAC,FIFAC,                                  & 
                                IRCNTSI,IRCNTMI,IRCNTFI,                            & 
                                IRAMTSI,IRAMTMI,IRAMTFI)                              
    ...
    IF(IRR_ACTIVE)THEN
      ...
      SMCAVL = (SH2O(1)-parameters%SMCWLT(1))*(-1.0)*ZSOIL(1)              
      SMCLIM = (parameters%SMCREF(1)-parameters%SMCWLT(1))*(-1.0)*ZSOIL(1) 
  ...
  SUBROUTINE SPRINKLER_IRRIGATION(parameters,NSOIL,DT,SH2O,SMC,     & 
                                  T2,WINDU,WINDV,EAIR,SIFAC,        & 
                                  IRAMTSI,IREVPLOS,IRSIRATE)          
    ...
    TEMP_RATE = parameters%SPRIR_RATE*(1/1000.0)*DT/3600.0         
    IRSIRATE  = MIN(FSUR*DT,IRAMTSI,TEMP_RATE)                   
                                                                 
    
    
    
    
    WINDSPEED = SQRT((WINDU**2.0)+(WINDV**2.0))                                 
    ESAT1     = 610.8*EXP((17.27*(T2-273.15))/(237.3+(T2-273.15)))              
    IF(T2 .GT. 273.15)THEN 
    ...
    IF ( isnan_lsm(IRRLOSS) ) IRRLOSS=4.0 
    IF ( (IRRLOSS .GT. 100.0) .OR. (IRRLOSS .LT. 0.0) ) IRRLOSS=4.0 
  ...
  SUBROUTINE MICRO_IRRIGATION(parameters,NSOIL,DT,SH2O,SMC,SICE,MIFAC, & 
                              IRAMTMI,IRMIRATE)                          
    ...
    TEMP_RATE = parameters%MICIR_RATE*(1.0/1000.0)*DT/3600.0        
    IRMIRATE  = MIN(0.5*FSUR*DT,IRAMTMI,TEMP_RATE)               
  ...
  SUBROUTINE FLOOD_IRRIGATION(parameters,NSOIL,DT,SH2O,SMC,SICE,FIFAC,& 
                              IRAMTFI,IRFIRATE)                         
    ...
    IRFIRATE  = FSUR*DT*parameters%FIRTFAC                
  ...
  SUBROUTINE IRR_PHILIP_INFIL(parameters,SMC,SH2O,SICE,DT,NSOIL, & 
                              FSUR)                                
  ...
  REAL, PARAMETER                             :: CMIC = 0.80 
      ...
      FFF   = parameters%BEXP(IWT) / 3.0 
      RSBMX = HK(IWT) * 1.0e3 * EXP(3.0) 
      ...
      SMPFZ  = -parameters%PSISAT(IWT)*1000.0*S_NODE**(-parameters%BEXP(IWT))   
      ...
      IF(IWT.EQ.NSOIL) THEN
         WA          = WA + (QIN - QDIS) * DT     
         WT          = WA
         ZWT         = (-ZSOIL(NSOIL) + 25.0) - WA/1000.0/ROUS      
      ...
      ELSE
         ...
         IF (IWT.EQ.NSOIL-1) THEN
            ZWT = -ZSOIL(NSOIL)                   &
                 - (WT-ROUS*1000*25.0) / (EPORE(NSOIL))/1000.0
         ELSE
            WS = 0.0   
            DO IZ = IWT+2,NSOIL
...
END  SUBROUTINE SHALLOWWATERTABLE
  ...
  real :: r,x
    ...
    STMSMN  = xsamin/sapm
     ...
     RSLEAF  = MIN((LFMASS-LFMSMN)/DT,RESP*12.0e-6)                         
     
     RSROOT  = parameters%RMR25*(RTMASS*1E-3)*TF *RF* 12.0e-6         
     RSSTEM  = parameters%RMS25*((STMASS-STMSMN)*1E-3)*TF *RF* 12.0e-6         
     ...
     HETERS = 0.9*RSSOIL                                         
     NEE    = (AUTORS + HETERS - GPP)*44.0/12.0                  
     TOTSC  = FASTCP + STBLCP                                    
 ...
 SUBROUTINE CARBON_CROP (parameters,NSNOW  ,NSOIL  ,VEGTYP ,DT     ,ZSOIL  ,JULIAN , & 
  ...
  real :: r,x
    ...
    STMSMN  = xsamin/sapm
     ...
     RSLEAF  = MIN((LFMASS-LFMSMN)/DT,RESP*30.0e-6)                       
     RSROOT  = parameters%RTMR25*(RTMASS*1E-3)*TF * 30.0e-6         
     RSSTEM  = parameters%STMR25*(STMASS*1E-3)*TF * 30.0e-6         
     RSGRAIN = parameters%GRAINMR25*(GRAIN*1E-3)*TF * 30.0e-6       
     ...
     HETERS = RSSOIL                                             
     NEE    = (AUTORS + HETERS - GPP)*44.0/30.0                    
     TOTSC  = FASTCP + STBLCP                                    
...
END SUBROUTINE EMERG
  ...
  INTEGER,  INTENT(IN) :: iopt_irrm 
                                    
                                    
                                    
  INTEGER,  INTENT(IN) :: iopt_infdv
  INTEGER,  INTENT(IN) :: iopt_tdrn 
...
MODULE NOAHMP_TABLES
    ...
    REAL :: BVIC_TABLE(MAX_SOILTYP)        
    REAL :: AXAJ_TABLE(MAX_SOILTYP)        
    REAL :: BXAJ_TABLE(MAX_SOILTYP)        
    REAL :: XXAJ_TABLE(MAX_SOILTYP)        
    REAL :: BDVIC_TABLE(MAX_SOILTYP)       
    REAL :: GDVIC_TABLE(MAX_SOILTYP)       
    REAL :: BBVIC_TABLE(MAX_SOILTYP)       
    ...
    REAL :: Z0SNO_TABLE    
    REAL :: SSI_TABLE      
    REAL :: SNOW_RET_FAC_TABLE  
    REAL :: SNOW_EMIS_TABLE
    REAL :: SWEMX_TABLE    
    REAL :: TAU0_TABLE          
 ...
 INTEGER :: IRR_HAR_TABLE               
    ...
    REAL :: IRR_MAD_TABLE               
    REAL :: FILOSS_TABLE                
    REAL :: SPRIR_RATE_TABLE            
    REAL :: MICIR_RATE_TABLE            
    REAL :: FIRTFAC_TABLE               
    REAL :: IR_RAIN_TABLE               
...
"STOP in Noah-MP read_mp_crop_parameters")
 ...
 DEFAULT_CROP_TABLE      = DEFAULT_CROP
  ...
  subroutine read_mp_irrigation_parameters()
    ...
    IRR_FRAC_TABLE   = -1.0E36    
    IRR_HAR_TABLE    =  0        
    IRR_LAI_TABLE    = -1.0E36    
    IRR_MAD_TABLE    = -1.0E36    
    FILOSS_TABLE     = -1.0E36    
    SPRIR_RATE_TABLE = -1.0E36    
    MICIR_RATE_TABLE = -1.0E36    
    FIRTFAC_TABLE    = -1.0E36    
    IR_RAIN_TABLE    = -1.0E36    
...
"STOP in NoahMP_irrigation_parameters")
    endif
...
"STOP in Noah-MP read_tiledrain_parameters")
    endif
